...
function humanPeriod(recordDate) {
  ...
  return {
    ...
    axis: `${year}.${String(month).padStart(2, '0')}`,
  };
}
  ...
  return (
    <div className="space-y-11">
      ...
      >
        ...
        <div className="text-[2.6rem] leading-tight mt-3" style={{ fontWeight: 700, letterSpacing: '-0.02em', ...TAB }}>
          {privacy ? mask(won(latestNW)) : won(latestNW)}
        </div>
        <div className="flex gap-9 mt-6">
          <div>
            <div className="text-xs mb-1 flex items-center gap-0.5" style={{ color: C.muted }}>전년 대비 <ChevronRight size={11} /></div>
            <div className="text-sm" style={{ fontWeight: 600, color: yoyDiff >= 0 ? C.mint : C.red, ...TAB }}>
              {yoyDiff !== null ? `${manSigned(yoyDiff)} · ${pctSigned(yoyPct)}` : '\u2014'}
            </div>
          </div>
          <div
          ...
          >
            ...
            <div className="text-sm" style={{ fontWeight: 600, color: latestPnl >= 0 ? C.mint : C.red, ...TAB }}>
              {privacy ? mask(moneySigned(latestPnl)) : moneySigned(latestPnl)}
            </div>
          </div>
        </div>
      </button>
      ...
      <div>
        ...
        <div style={{ height: 210, marginTop: 20 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={growthSeries} margin={{ top: 10, right: 10, left: 6, bottom: 0 }}>
              <CartesianGrid stroke="#161F33" vertical={false} />
              <XAxis dataKey="axis" stroke={C.faint} fontSize={10} tickLine={false} axisLine={false} />
              <YAxis
                hide
                domain={[
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-5">
          {BAR_PNL_CATEGORIES.map((c) => {
            ...
            const share = pnlShareByKey[c.label] ?? 0;
            return (
              <div key={c.key}>
                <div className="flex items-center gap-1.5 text-xs mb-1.5" style={{ color: C.muted }}>
                  <span className="rounded-full" style={{ width: 6, height: 6, backgroundColor: c.color, display: 'inline-block' }} />
                  {c.label}
                </div>
                <div className="text-sm" style={{ fontWeight: 700, color: val > 0 ? C.mint : val < 0 ? C.red : C.soft, ...TAB }}>
                  {privacy ? mask(moneySigned(val)) : moneySigned(val)}
                </div>
                <div className="text-[11px] mt-0.5" style={{ color: C.faint }}>{share}%</div>
              </div>
            );
          })}
        </div>

        <div className="flex justify-between mt-6 rounded-xl px-4 py-3.5 text-xs" style={{ backgroundColor: C.cardAlt }}>
          <div>
            <div className="mb-1" style={{ color: C.muted }}>이익</div>
            <div style={{ fontWeight: 600, color: C.mint, ...TAB }}>{moneySigned(pnlProfit)}</div>
          </div>
          <div>
            <div className="mb-1" style={{ color: C.muted }}>손실</div>
            <div style={{ fontWeight: 600, color: pnlLoss > 0 ? C.red : C.white, ...TAB }}>
              {pnlLoss > 0 ? `-${won(pnlLoss)}` : `+${won(0)}`}
            </div>
          </div>
          <div>
            <div className="mb-1" style={{ color: C.muted }}>순손익</div>
  ...
  return (
    <div className="space-y-9">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div style={{ ...EYEBROW, color: C.mint, marginBottom: 10 }}>YEAR-END RECORDS</div>
          <h1 className="text-[28px]" style={DISPLAY}>해마다, 한 장씩.</h1>
            ...
            <div key={r.id} className="rounded-xl p-5" style={{ border: `1px solid ${C.border}`, backgroundColor: C.card }}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-xl mt-2.5" style={{ fontWeight: 700, ...TAB }}>
    ...
    <div className="space-y-11">
      ...
      <div>
        ...
        <div style={{ height: 200, marginTop: 18 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={netWorthLineSeries} margin={{ top: 10, right: 10, left: 6, bottom: 0 }}>
              <CartesianGrid stroke="#161F33" vertical={false} />
              <XAxis dataKey="axis" stroke={C.faint} fontSize={10} tickLine={false} axisLine={false} />
              <YAxis
                hide
      ...
      <div>
        ...
        <div style={{ height: 170, marginTop: 18 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={pnlBarSeries} margin={{ top: 24, right: 10, left: 6, bottom: 0 }}>
              <CartesianGrid stroke="#161F33" vertical={false} />
              <XAxis dataKey="year" stroke={C.faint} fontSize={10} tickLine={false} axisLine={false} />
              <YAxis hide />
      ...
      {changeBreakdown && (
        <div className="rounded-xl p-5" style={{ border: `1px solid ${C.border}`, backgroundColor: C.card }}>
          ...
          <div className="text-2xl mt-2" style={{ fontWeight: 700, color: changeBreakdown.total >= 0 ? C.mint : C.red, ...TAB }}>
...
function RecordEditor({ editing, setEditing, onSave, onClose }) {
  ...
  function setAsset(key, value) { setEditing({ ...editing, assets: { ...editing.assets, [key]: Number(value) || 0 } }); }
  function setPnl(key, value) { setEditing({ ...editing, pnl: { ...editing.pnl, [key]: Number(value) || 0 } }); }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
      <div className="w-full max-w-md rounded-t-2xl overflow-y-auto" style={{ backgroundColor: C.bgAlt, borderTop: `1px solid ${C.border}`, maxHeight: '92vh' }}>
        ...
        <div className="p-5 space-y-6">
          ...
          <p className="text-[11px] -mt-3" style={{ color: C.faint }}>상반기 기록은 6월, 하반기 기록은 12월을 기준월로 남기면 표기가 자연스러워요.</p>
            ...
            <div className="space-y-3">
              {CATEGORIES.map((c) => (
                <div key={c.key} className="flex items-center gap-3">
                  ...
                  <input type="number" value={editing.assets[c.key] || ''} onChange={(e) => setAsset(c.key, e.target.value)} placeholder="0"
                    className="flex-1 px-3 py-2 text-sm text-right" style={inputStyle} />
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {PNL_CATEGORIES.map((c) => (
                <div key={c.key} className="flex items-center gap-3">
                  <span className="rounded-full shrink-0" style={{ width: 8, height: 8, backgroundColor: c.color, display: 'inline-block' }} />
                  <span className="text-sm shrink-0" style={{ color: C.soft, width: 96 }}>{c.label}</span>
                  <input type="number" value={editing.pnl[c.key] || ''} onChange={(e) => setPnl(c.key, e.target.value)} placeholder="0"
                    className="flex-1 px-3 py-2 text-sm text-right" style={inputStyle} />
                </div>
              ))}
            </div>
